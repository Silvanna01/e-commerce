...
const getProducts = async (page = 1) => {
  try {
    const response = await fetch(`${BASE_URL}/v1/products?page=${page}`);
...
const createCard = (item) => {
  const { id, imageUrl, productName, price } = item;

  const articleEl = document.createElement('article');
  articleEl.setAttribute('data-productId', id);
...
const addToCart = (item) => {
  ...
  alert(`${item.goods_name} has been added to the cart`)
}
